 ...
 fun t v ->
  ...
  | Dict (basic, typ) -> (
      ...
      | Dict xs -> (
          ...
          | String ->
              ...
              let vs = tailrec_map snd xs in
              list_helper typ vs >>= fun vs -> return (List.combine keys vs)
          | _ -> Error (`Msg "Expecting something other than a Dict type"))
      | _ -> Error (`Msg "Unhandled"))
  | List typ -> (
      match v with
  ...
  | Option t -> (
      ...
      | y ->
          Error
            (`Msg
              (Printf.sprintf "Expecting an Enum value, got '%s'"
                 (Rpc.to_string y))))
  | Tuple (t1, t2) -> (
      match (v, t2) with
      ...
      | Rpc.Enum [ x; y ], _ ->
          ...
          unmarshal t2 y >>= fun v2 -> Ok (v1, v2)
      | Rpc.Enum _, _ -> Error (`Msg "Too many items in a tuple!")
      | _, _ -> Error (`Msg "Expecting Rpc.Enum when unmarshalling a tuple"))
  | Tuple3 (t1, t2, t3) -> (
      match v with
      | Rpc.Enum [ x; y; z ] ->
          unmarshal t1 x >>= fun v1 ->
          unmarshal t2 y >>= fun v2 ->
          unmarshal t3 z >>= fun v3 -> Ok (v1, v2, v3)
      | Rpc.Enum _ ->
          Error (`Msg "Expecting precisely 3 items when unmarshalling a Tuple3")
      | _ -> Error (`Msg "Expecting Rpc.Enum when unmarshalling a tuple3"))
  | Tuple4 (t1, t2, t3, t4) -> (
      match v with
      | Rpc.Enum [ x; y; z; a ] ->
          unmarshal t1 x >>= fun v1 ->
          ...
          unmarshal t4 a >>= fun v4 -> Ok (v1, v2, v3, v4)
      | Rpc.Enum _ ->
          Error
            (`Msg
              "Expecting precisely 4 items in an Enum when unmarshalling a \
               Tuple4")
      | _ -> Error (`Msg "Expecting Rpc.Enum when unmarshalling a tuple4"))
  | Struct { constructor; sname; _ } -> (
      ...
      | Rpc.Dict keys' ->
          ...
          constructor
            {
              field_get =
                (let x : type a. string -> a typ -> (a, Rpc.msg) result =
                  fun s ty ->
                   let s = String.lowercase_ascii s in
                   match ty with
                   | Option x -> (
                       try
                         List.assoc s keys |> unmarshal x >>= fun o ->
                         return (Some o)
                       with _ -> return None)
                   | y -> (
      ...
      | _ ->
          Error
            (`Msg
              (Printf.sprintf "Expecting Rpc.Dict when unmarshalling a '%s'"
                 sname)))
  | Variant { vconstructor; _ } ->
      ...
      | _ ->
          Error (`Msg "Expecting String or Enum when unmarshalling a variant"))
      >>= fun (name, contents) ->
