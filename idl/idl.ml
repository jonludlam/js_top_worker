...
let get_arg call has_named name is_opt =
  ...
  | true, Some n, Rpc.Dict named :: unnamed -> (
      ...
      | [], _others when is_opt -> Ok (Rpc.Enum [], call)
      | _, _ -> Error (`Msg (Printf.sprintf "Expecting named argument '%s'" n)))
  | true, None, Rpc.Dict named :: unnamed -> (
      match unnamed with
      | head :: tail ->
          Ok (head, { call with Rpc.params = Rpc.Dict named :: tail })
      | _ -> Error (`Msg "Incorrect number of arguments"))
  | true, _, _ ->
      Error
        (`Msg
          "Marshalling error: Expecting dict as first argument when named \
           parameters exist")
  | false, None, head :: tail -> Ok (head, { call with Rpc.params = tail })
  | false, None, [] -> Error (`Msg "Incorrect number of arguments")
...
module Make (M : MONAD) = struct
  ...
  module GenClient () = struct
    ...
    let declare_ is_notification name _ ty (rpc : T.rpcfn) =
      let rec inner :
          type b. (string * Rpc.t) list option * Rpc.t list -> b fn -> b =
            ...
            let res =
              T.bind rpc (fun r ->
                  if r.Rpc.success then
                    match
                      Rpcmarshal.unmarshal t.Param.typedef.Rpc.Types.ty
                        r.Rpc.contents
                    with
                    | Ok x -> M.return (Ok x)
  ...
  let server hashtbl =
    ...
    fun call ->
      let fn =
        try Hashtbl.find impl call.Rpc.name
        with Not_found ->
          !logfn "1";
          Hashtbl.iter
            (fun key _ ->
              !logfn ("method: " ^ key ^ (Hashtbl.hash key |> string_of_int));
              !logfn key)
            impl;
          let _h = Hashtbl.hash call.Rpc.name in

          !logfn
            (Printf.sprintf "Unknown method: %s %d" call.Rpc.name
               (Hashtbl.hash call.Rpc.name));
          !logfn call.Rpc.name;
          ...
          let rec inner : type a. a fn -> a -> T.rpcfn =
           fun f impl call ->
            ...
            | Function (t, f) -> (
                ...
                | Ok (x, y) -> M.return (x, y)
                | Error (`Msg m) -> M.fail (MarshalError m))
                >>= fun (arg_rpc, call') ->
                let z =
                  Rpcmarshal.unmarshal t.Param.typedef.Rpc.Types.ty arg_rpc
                in
                match z with
                | Ok arg -> inner f (impl arg) call'
                | Error (`Msg m) -> M.fail (MarshalError m))
            | Returning (t, e) ->
...
module DefaultError = struct
    ...
    let open Rpc.Types in
    {
      ...
      vconstructor =
        (fun s t ->
          match s with
          | "InternalError" -> (
              match t.tget (Basic String) with
              | Ok s -> Ok (internalerror.treview s)
              | Error y -> Error y)
          | s -> Error (`Msg (Printf.sprintf "Unknown tag '%s'" s)));
    }
  ...
  let err =
    let open Error in
    {
      ...
      matcher =
        (function InternalErrorExn s -> Some (InternalError s) | _ -> None);
    }
end
       ...
       fun (named, unnamed) -> function
        ...
        | Returning (t, e) -> (
            ...
            if r.Rpc.success then
              ...
              match
                Rpcmarshal.unmarshal e.Error.def.Rpc.Types.ty r.Rpc.contents
              with
          ...
          let rec inner : type a. a fn -> a -> Rpc.call -> Rpc.response =
           fun f impl call ->
            try
              ...
              | Function (t, f) ->
                  ...
                  let arg_rpc, call' =
                    match get_arg call has_named t.Param.name is_opt with
                    | Ok (x, y) -> (x, y)
                    | Error (`Msg m) -> raise (MarshalError m)
                  in
                  let z =
                    Rpcmarshal.unmarshal t.Param.typedef.Rpc.Types.ty arg_rpc
                  in
            ...
            with e -> (
              ...
              | Some y ->
                  Rpc.failure
                    (Rpcmarshal.marshal error_ty.Error.def.Rpc.Types.ty y)
              | None -> raise e)
          in
          inner ty impl
        in
